...
def lidar_envio_arquivo_diretorio(instancia_servidor, conn, id_cliente, ip_cliente, caminho_armazenamento_cliente, comando):
    ...
    # Verifica se o comando está bem formado Exemplo: "UPLOAD:true:caminho/relativo:123456" ou "UPLOAD:false:caminho/relativo"
    try:
        ...
        eh_diretorio_str = partes[1] #Primeira parte após o comando "UPLOAD"
        caminho_relativo = partes[2] # Segunda parte após o comando "UPLOAD"
        tamanho_ou_marcador_dir = partes[3] # Terceira parte após o comando "UPLOAD", que pode ser o tamanho do arquivo ou um marcador para diretório

        eh_diretorio = eh_diretorio_str.lower() == 'true' # Verifica se é um diretório

        # Normaliza o caminho para lidar com caracteres especiais Exemplo: "caminho/relativo/arquivo.txt" ou "caminho/relativo/diretorio/" TÁ BUGADO
        ...
        if eh_diretorio: 
                ...
                with open(caminho_destino, 'wb') as arquivo_destino: 
                    while bytes_recebidos < tamanho_arquivo:
                        ...
                        if not dados:
                            ...
                            #Throughput: taxa de transferência = tamanho do arquivo / tempo de transferência
                            taxa_transferencia_mbps = (bytes_recebidos / (1024 * 1024)) / (tempo_decorrido + 0.001)  # IA: Evita divisão por zero
            ...
            #IA: Tratamento de exceções para erros comuns durante o upload
            except ValueError:
...
def lidar_definir_privacidade(instancia_servidor, conn, id_cliente, comando):
    ...
    try:
        with instancia_servidor.trava_clientes[id_cliente]:# Bloqueia o acesso ao dicionário de clientes para evitar condições de corrida
            config_privacidade = comando.split(":")[1].lower()# Verifica se o comando está bem formado Exemplo: "PRIVACIDADE:publica" ou "PRIVACIDADE:privada"
...
def lidar_listar_pastas_publicas(instancia_servidor, conn, id_cliente_solicitante):
    ...
    for id_cliente_na_lista, dados_cliente_na_lista in instancia_servidor.dados_clientes.items():# Itera sobre os clientes no dicionário de clientes
...
def lidar_listar_arquivos(instancia_servidor, conn, id_cliente_atual, caminho_armazenamento_cliente, comando):
    ...
    id_cliente_alvo = comando.split(":")[1]# Verifica se o comando está bem formado Exemplo: "LISTAR:123456:caminho/relativo" ou "LISTAR:123456:" (sem caminho relativo)
    caminho_para_listar = ""
    ...
    diretorio_base = ""
    if id_cliente_alvo == id_cliente_atual:# Se o cliente atual for o mesmo que o cliente alvo, usa o caminho de armazenamento do cliente atual
        diretorio_base = caminho_armazenamento_cliente
    else:
        if id_cliente_alvo in instancia_servidor.dados_clientes:# Verifica se o cliente alvo existe no dicionário de clientes
            # Se o cliente alvo não for o mesmo que o cliente atual, verifica se a pasta é pública
    ...
    caminho_completo_no_servidor = os.path.join(diretorio_base, caminho_para_listar) # Normaliza o caminho para lidar com caracteres especiais  ## TÁ BUGADO ##
    ...
    for item in os.listdir(caminho_completo_no_servidor):# Lista os itens no diretório especificado
    
        caminho_item = os.path.join(caminho_completo_no_servidor, item)
    
        eh_diretorio_item = os.path.isdir(caminho_item)
    
        if eh_diretorio_item:
            tamanho = 0
        else:
            tamanho = os.path.getsize(caminho_item)#GETsize retorna o tamanho do arquivo em bytes
